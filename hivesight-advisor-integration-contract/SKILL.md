...
## The Contract (current state: 2026-08-06, Slices 0008–0011 implemented and merged)

All three Advisor-side responses share `contract_version: "treatment_plan_v1"` and carry `answer_id` for audit correlation (Slice 0011). The treatment-plans request takes `jurisdiction_code` (a stable string, e.g. `"uk"`/`"us"`), **not** an internal Advisor UUID — this was a breaking change from the original shape, made deliberately before any real caller existed. `request_treatment_plan` is idempotent per hive: a repeated request while a suggestion is still `suggested` returns that same suggestion instead of creating a competing one; a repeated request after the prior one was `completed` starts a genuinely new episode.

| Integration point | Direction | Status |
|---|---|---|
| Request a treatment plan for a hive | HiveSight → Advisor | **Built and tested on the Advisor side.** `POST /integrations/hivesight/treatment-plans` — body `{hive_id, jurisdiction_code, situational_context}`, returns `{contract_version, answer_id, text, grounding_status, citations}`. Unknown `jurisdiction_code` returns 422. Not yet called by a real HiveSight caller. |
...
| Confirm a treatment was completed | HiveSight → Advisor | **Stood in for, on the Advisor side, by a test-only endpoint.** `POST /integrations/hivesight/treatment-plans/completions` — body `{hive_id}`, returns `{contract_version, id, answer_id, status}` or 404 if nothing is awaiting completion. Simulates the eventual real HiveSight webhook; HiveSight has not built the real call yet. |
| Reject a suggested treatment, with a reason | HiveSight → Advisor | **Stood in for, on the Advisor side, by a test-only endpoint (Slice 0009).** `POST /integrations/hivesight/treatment-plans/rejections` — body `{hive_id, reason}`, returns `{contract_version, answer_id, text, grounding_status, citations, revision_exhausted}` or 404 if nothing is awaiting completion. Below a cap of 3 revisions, this produces a genuinely revised suggestion (the Advisor's graph loops back to its own recommend step); at the cap, `revision_exhausted: true` is returned and the *previous* suggestion remains acceptable via the completion endpoint above — rejection never forecloses accepting what's already on offer. Simulates HiveSight's eventual real rejection webhook, which does not exist yet. |
...
**Still open on HiveSight's side, flagged during Slice 0029 review, not part of Slice 0011**: `X-HiveSight-Service-Key` is currently proposed as the header name HiveSight itself would check on inbound Advisor calls too — the same name Advisor already uses for the opposite direction. Two different secrets, one name, in two directions is a real footgun; HiveSight should pick a direction-specific name (e.g. `X-Advisor-Service-Key`) for its own inbound header.
